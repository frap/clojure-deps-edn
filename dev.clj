...
(defn- start-repl
  ...
  * start a plain ol' Clojure REPL.
  ...
  (try
    ...
    (println "Emacs is running")
    (catch Throwable _))
  

  (let [s-port (or (->long (System/getenv "NREPL_PORT"))
                   (->long (System/getProperty "nrepl-port"))
                   (->long (try (slurp ".nrepl-port") (catch Throwable _)))
                   50505)]
    ...
    (when-not (get (deref (requiring-resolve 'clojure.core.server/servers)) "repl")
      (println "Selected port" s-port "for the nREPL...")
      (spit ".nrepl-port" (str s-port))
      (try
        ((requiring-resolve 'nrepl.server/start-server)
         ...
         {:port s-port :name (str "nREPL-" s-port)
